...
ser = serial.Serial('COM5', 115200) #COM5改成/dev/ttyUSB0即可，在linux虚拟机里运行
...
def angle_cos(p0, p1, p2):
    ...
    参数:
    p0 (numpy.ndarray): 第一个点的坐标。
    p1 (numpy.ndarray): 第二个点的坐标。
    p2 (numpy.ndarray): 第三个点的坐标。
...
def are_rectangles_close(rect1, rect2):
    ...
    该函数通过比较两个矩形的四个角点来判断它们是否足够接近。如果两个矩形的四个角点的距离都在阈值之内，则认为它们是接近的。

    参数:
    rect1 (numpy.ndarray): 第一个矩形的四个角点坐标。
    rect2 (numpy.ndarray): 第二个矩形的四个角点坐标。

    返回:
    bool: 如果两个矩形足够接近，则返回 True，否则返回 False。
    """
    # 定义一个阈值，用于判断两个角点是否足够接近
    threshold = 2

    # 使用 zip 函数同时遍历两个矩形的角点
    for p1, p2 in zip(rect1, rect2):
        # 检查每个角点的 x 和 y 坐标的差值是否都在阈值之内
        if not (abs(p1[0][0] - p2[0][0]) <= threshold and abs(p1[0][1] - p2[0][1]) <= threshold):
            # 如果有任何一个角点的距离超过阈值，则返回 False
            return False

    # 如果所有角点的距离都在阈值之内，则返回 True
    return True
...
def is_laser_between_rectangles(laser_point, outer_rect, inner_rect):
    # 判断激光点是否在外层矩形内
    outer_check = cv2.pointPolygonTest(outer_rect, laser_point, False)
    # 判断激光点是否在内层矩形内
    inner_check = cv2.pointPolygonTest(inner_rect, laser_point, False)

    # 如果激光点在外层矩形内且不在内层矩形内，认为激光点位于两个轮廓之间
    if outer_check >= 0 and inner_check < 0:
        return True
    return False
...
def find_rectangles(image):
    ...
    for i, (cnt, hier) in enumerate(zip(contours, hierarchy[0])):
        ...
        if len(approx) == 4 and cv2.contourArea(approx) > 1000:
            ...
            if all(cos < 0.2 for cos in cosines):  # 检查角度接近90度
                ...
                if not too_close:
                    # 判断是否为外层轮廓
                    is_outer = hier[3] == -1  # 如果父轮廓为-1，则是外层轮廓

                    # 保存角点
                    rectangles_data.append(approx_sorted)

                    # 在原图上标出矩形
                    color = (0, 0, 255) if is_outer else (0, 255, 0)  # 外层红色，内层绿色
                    cv2.drawContours(img_contour, [approx_sorted], -1, color, 1)

                    for point in approx:
                        cv2.circle(frame, tuple(point[0]), 10, (255, 255, 0), -1)
...
def green_laser_detection(image):
    ...
    if contours:
        ...
        cv2.circle(img_contour, green_laser_point, 5, (0, 255, 0), -1)
...
def move_point_level(velocity, direction):
    ...
    参数:
    velocity (int): 点移动的速度。
    direction (int): 点移动的方向，1 表示向右或向下，-1 表示向左或向上。

    返回:
    None
    """
    # 更新点的 x 坐标，根据速度和方向进行移动
    point_level[0] += velocity * direction
    # 在图像上绘制移动后的点
    cv2.circle(img_contour, point_level, 2, (0, 0, 0), thickness=-1)
...
while True:
    ...
    if len(rectangles) >= 2:
        area1 = cv2.contourArea(rectangles[0])
        area2 = cv2.contourArea(rectangles[1])
        if area1 > area2:
    ...
    if len(rectangles) >= 2:
        if red_laser:
            ...
            cv2.putText(img_contour, f"red:{red_laser}", (20, 90),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1, cv2.LINE_AA)
            # ser.write(f"@red={red_laser}!".encode())
        if green_laser:
            print("green", green_laser)
            cv2.putText(img_contour, f"green:{green_laser}", (20, 130),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1, cv2.LINE_AA)
            # ser.write(f"@green={green_laser}!".encode('UTF-8'))
        if red_laser and green_laser:
            distance = np.linalg.norm(np.array(red_laser) - np.array(green_laser))
            ser.write(f"@distance={distance}\r\n".encode('UTF-8'))

        if red_laser and is_inside_polygon(red_laser, rectangles[0]):
            cv2.putText(img_contour, f"Rin", (20, 170),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1, cv2.LINE_AA)
        elif red_laser and not is_inside_polygon(red_laser, rectangles[0]) and is_inside_polygon(red_laser,
                                                                                                 rectangles[1]):
            cv2.putText(img_contour, f"Rbet", (20, 170),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1, cv2.LINE_AA)
        else:
            cv2.putText(img_contour, f"Rout", (20, 170),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1, cv2.LINE_AA)

        if green_laser and is_inside_polygon(green_laser, rectangles[0]):
            cv2.putText(img_contour, f"Gin", (20, 200),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1, cv2.LINE_AA)
        elif green_laser and not is_inside_polygon(green_laser, rectangles[0]) and is_inside_polygon(green_laser,
                                                                                                     rectangles[1]):
            cv2.putText(img_contour, f"Gbet", (20, 200),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1, cv2.LINE_AA)
        else:
            cv2.putText(img_contour, f"Gout", (20, 200),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1, cv2.LINE_AA)
    ...
    if (mode == 1) and len(rectangles) >= 2:
        ...
        # cv2.drawContours(img_contour, [average_rect], -1, (0, 0, 0), 1)
